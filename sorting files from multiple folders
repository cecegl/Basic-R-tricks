...
#This will create a list of directory paths to all the subfolders in your main folder
dir_path <- list.dirs(path = "path to main repository (with multiple folders)",
                      recursive = FALSE)

#This will search all your subfolders one by one for files of the type you select and then copy each of them to the desired destination
